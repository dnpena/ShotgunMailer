class SessionsController < ApplicationController
  def new
    if current_user
      return redirect_to inbox_path, :notice => "Already logged in!"
    end
    @user = User.new
  end
  ...
  def unsubscribe
    ...
    @token.destroy
    redirect_to inbox_path, notice: t('notice.user.unsubscribe')
  end
  ...
  def send_pass_recovery
    ...
    if user
      ...
      return redirect_to inbox_path, :notice => t('notice.sessions.recovery')
    else
  ...
  def change_password
    @token = Token.find_by_value(params[:token])
    unless @token
      return redirect_to inbox_path, :notice => t('notice.sessions.invalid_link')
    end
    @user = @token.user
  ...
  def do_change_password
    ...
    Token.delete @token
    return redirect_to inbox_path, :notice => t('notice.sessions.pass_changed')
  end
  #end recover pass
